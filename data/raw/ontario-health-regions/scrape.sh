...
echo HKPR $(wget -q -O - https://www.hkpr.on.ca/covid-19-2/covid-19/  | grep HKPRDHU | head -n 3 | tail -n 1 | sed -E -e 's/^.*Confirmed Cases<.......[^>]*>[0-9]*........[^>]*>[0-9]*........[^>]*>[0-9]*........[^>]*>([^<]+)<.*/\1/')
...
echo Leeds NA && wget -q https://web.archive.org/save/https://healthunit.org/coronavirus/
...
echo North-Bay-Parry-Sound $(wget -q -O - https://www.myhealthunit.ca/en/health-topics/coronavirus.asp | grep td | grep Right | head -n 1 | sed -E -e  's/.*Right.>([^>]+)<.*$/\1/')
...
##
wget -q https://web.archive.org/save/https://www.quebec.ca/en/health/health-issues/a-z/2019-coronavirus/situation-coronavirus-in-quebec/
wget -q https://web.archive.org/save/http://www.bccdc.ca/health-info/diseases-conditions/covid-19/case-counts-press-statements
